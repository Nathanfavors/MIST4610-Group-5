...
2.  Analyzing Member Reservations (Complex) - This query returns the member name and counts the courts the member has reserved, showing the member with the most courts reserved first.This query can help the manager understand which members are utilizing the tennis courts the most and at what times. It provides insights into member activity, allowing the manager to optimize court scheduling and potentially identify trends for targeted promotions or adjustments in offerings. 
...
3.  Managing Inventory (Complex) -  This query returns an individual item along with the number of times it has been rented, its overall revenue, and the quantity of the item in stock. This query can help the manager understand the frequency with which different items are being rented and when. It provides insight into what items are useful to carry or may not be necessary to carry.
